// Step 17: Market execution ports — interfaces/types only.
//
// Three distinct ports required for execution gate evaluation and order construction:
//   1. ExecutionQuotePort    – fresh quote with provenance and staleness guarantee
//   2. ExecutionSessionPort  – Asia/Kolkata exchange-session status and window
//   3. BrokerMappingPort     – immutable instrument-to-broker mapping snapshots
//
// All results must carry sufficient provenance (source, timestamps, hashes) for the
// execution gates to verify freshness and immutability before placement.
...
export interface ExecutionQuotePort {
  ...
  ): Promise<DomainResult<ExecutionQuoteSnapshot>>
}
...
export interface ExecutionSessionPort {
  ...
  ): Promise<DomainResult<ConfirmedExecutionSession>>
}
...
export interface BrokerMappingPort {
   ...
   */
  loadMapping(
    request: MappingLoadRequest,
  ): Promise<DomainResult<MappingLoadResult>>
   ...
   */
  loadMappingBatch(
    requests: readonly MappingLoadRequest[],
