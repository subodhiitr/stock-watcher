// Step 16: Plan and portfolio state ports — interfaces/types only.
//
// Async read ports that assemble execution context from U01 (portfolio accounting),
// U03 (policy/corporate-action lineage), U04 (rebalance plan), and current U05
// aggregate state (approval, run, reconciliation, kill-switch).
//
// All reads happen OUTSIDE the synchronous transaction boundary so that the
// broker calls, timer waits, and large state loads do not block the commit path.
...
export interface ExecutionStatePort {
   ...
   */
  loadCurrentPlan(
    request: PlanStateRequest,
  ): Promise<DomainResult<CurrentPlanState | undefined>>

  /**
   * Load U01 portfolio accounting including holdings, cash, and reservations.
   */
  loadPortfolioAccounting(
    request: PortfolioAccountingRequest,
  ): Promise<DomainResult<ExecutionPortfolioAccounting>>
   ...
   */
  loadPolicyLineage(
    request: PolicyLineageRequest,
  ): Promise<DomainResult<ExecutionPolicyLineage>>

  /**
   * Load U03 corporate-action evidence for the portfolio's execution date.
   */
  loadCorporateActionEvidence(
    request: CorporateActionEvidenceRequest,
  ): Promise<DomainResult<ExecutionCorporateActionEvidence>>

  /**
   * Load current U05 aggregate lineage: approval, active run, latest
   * reconciliation, and kill-switch state.
   */
  loadAggregateLineage(
    request: AggregateLineageRequest,
  ): Promise<DomainResult<ExecutionAggregateLineage>>
}
