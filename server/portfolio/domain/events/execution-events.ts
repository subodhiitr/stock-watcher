...
export type ExecutionEventAggregateKind =
  | 'PORTFOLIO'
  | 'APPROVAL'
  | 'EXECUTION_RUN'
  | 'EXECUTION_ORDER'
...
export type ExecutionEventFactKind =
...
export type ExecutionEventScope =
  | Readonly<{ kind: 'PORTFOLIO'; portfolioId: PortfolioId }>
  | Readonly<{ kind: 'GLOBAL'; globalStreamId: 'GLOBAL_EXECUTION_CONTROL' }>
...
export type ExecutionDomainEvent =
  | ExecutionAggregateMutationEvent
  | ExecutionFactInsertionEvent
...
export function freezeExecutionDomainEvent<T extends ExecutionDomainEvent>(event: T): T {
  return deepFreeze(event) as T
}
