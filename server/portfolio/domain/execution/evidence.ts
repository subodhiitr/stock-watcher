...
// Evidence payloads are bounded and allowlisted: no credentials, tokens, account IDs,
// raw broker payloads/errors, paths, SQL, stack traces, or arbitrary text.
...
export type KillSwitchEvidencePayload =
...
export type ResidualWorkEvidencePayload = Readonly<{
  ...
  reason: 'PARTIAL_FILL' | 'REJECTED' | 'CANCELLED' | 'EXPIRED'
...
export type ExecutionEvidencePayload =
  | ApprovalEvidencePayload
  | ExecutionRunEvidencePayload
  | ExecutionRunPortfolioVersionEvidencePayload
  | OrderIntentEvidencePayload
  | SubmissionAttemptEvidencePayload
  | SubmissionOutcomeEvidencePayload
  | OrderStateChangedEvidencePayload
  | FillRecordedEvidencePayload
  | PortfolioAccountingEvidencePayload
  | CancellationEvidencePayload
  | ReconciliationEvidencePayload
  | ReconciliationStateChangedEvidencePayload
  | ReconciliationSnapshotRecordedEvidencePayload
  | KillSwitchEvidencePayload
  | RecoveryEvidencePayload
  | ResidualWorkEvidencePayload
  | AdjustmentProposalEvidencePayload

export type ExecutionEvidenceKind = ExecutionEvidencePayload['kind']

// Health evidence: allowed fields, no secrets or raw broker data
...
// Progress evidence: bounded counters for audit and monitoring
...
// Validate that a payload kind is in the allowlist (exhaustive guard)
export function isKnownEvidenceKind(kind: string): kind is ExecutionEvidenceKind {
  const known: ReadonlySet<string> = new Set<ExecutionEvidenceKind>([
    'APPROVAL_DECIDED',
    'EXECUTION_RUN_STATE_CHANGED',
    'EXECUTION_RUN_PORTFOLIO_VERSION_ADVANCED',
    'ORDER_INTENT_RECORDED',
    'SUBMISSION_ATTEMPT_STARTED',
    'SUBMISSION_OUTCOME_RECORDED',
    'ORDER_STATE_CHANGED',
    'FILL_RECORDED',
    'PORTFOLIO_ACCOUNTING_CHANGED',
    'CANCELLATION_REQUESTED',
    'CANCELLATION_OUTCOME_RECORDED',
    'RECONCILIATION_COMPLETED',
    'RECONCILIATION_STATE_CHANGED',
    'RECONCILIATION_SNAPSHOT_RECORDED',
    'KILL_SWITCH_ACTIVATED',
    'KILL_SWITCH_RESET',
    'RECOVERY_CLASSIFIED',
    'RESIDUAL_WORK_RECORDED',
    'ADJUSTMENT_PROPOSAL_RECORDED',
  ])
  return known.has(kind)
}

// Redact the last 6 characters of an identifier for bounded audit context
