...
import type { ActorId, AllocationId, CausationId, CommandId, CorrelationId, EventId, EvidenceId, HoldingId, HoldingLotId, IdempotencyKey, InstrumentId, OrderId, PortfolioId, RebalanceRunId, StrategyAssignmentId, StrategyId, StrategySleeveId, StrategyVersionId } from '../domain/shared/identifiers.ts';
...
export interface PortfolioTransaction {
    ...
    appendDomainEvents(events: readonly PortfolioDomainEvent[]): DomainResult<void, AnyDomainFailure>;
}
...
export interface StrategyEvidencePort {
    resolveEligibility(portfolioId: PortfolioId, strategyVersionId: StrategyVersionId, effectiveAt: Instant): Promise<DomainResult<StrategyEligibilityEvidence>>;
}
export type CommittedEventHandler<Event extends PortfolioDomainEvent> = (event: Event) => Promise<DomainResult<void>>;
export interface InternalEventBus {
    publish(events: readonly PortfolioDomainEvent[]): Promise<DomainResult<void>>;
    subscribe<Event extends PortfolioDomainEvent>(eventType: Event['type'], handler: CommittedEventHandler<Event>): () => void;
}
